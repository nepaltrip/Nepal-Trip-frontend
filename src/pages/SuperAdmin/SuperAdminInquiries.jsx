...
import {
    Search, Mail, MailOpen, CheckCircle2, Send, MapPin,
    ArrowLeft, Clock, Ban, Trash2, Lock, Unlock, AlignLeft
} from "lucide-react";
                ...
                filter === "all" ? true :
                    filter === "unread" ? inq.status === "unread" :
                        filter === "replied" ? inq.status === "replied" :
                            filter === "closed" ? inq.status === "closed" :
                                true;
                ...
                <div className="p-4 md:p-6 border-b border-border/40 space-y-4 shrink-0 bg-[#FDFBF7] z-20">
                    ...
                    <div className="flex items-center justify-between gap-2 w-full overflow-x-auto custom-scrollbar pb-1">
                        <div className="flex gap-1 md:gap-2 flex-1 shrink-0">
                            {/* ✨ Added "Closed" to the filter pills */}
                                    ...
                                    <>
                                        <div className="p-3 md:p-6 border-b border-border/40 bg-white shrink-0 z-10 shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
                                            <div className="flex items-start justify-between gap-4 mb-3">
                                                ...
                                                {/* ✨ NEW: Close / Reopen Toggle Button */}
                                                <button
                                                    onClick={() => handleToggleStatus(activeDisplay.status)}
                                                    disabled={isStatusToggling}
                                                    className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all shadow-sm active:scale-95 disabled:opacity-50 ${activeDisplay.status === 'closed' ? 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200' : 'bg-slate-50 text-slate-700 hover:bg-slate-100 border border-slate-200'}`}
                                                >
                                        ...
                                        <div className="p-3 md:p-6 border-t border-border/40 bg-[#FDFBF7] shrink-0 shadow-[0_-4px_20px_rgba(0,0,0,0.02)]">
                                            ...
                                            {activeDisplay.status === "closed" ? (
                                                ...
                                                <div className="flex flex-col gap-2 md:gap-3">
                                                    {/* ✨ NEW: Multi-Reply Banner (Doesn't hide the form) */}
