...
export default function SuperAdminUsers() {
    ...
    useEffect(() => {
        ...
        // Listen for the exact event emitted from server.js
            ...
            setUsers(prevUsers => prevUsers.map(u => {
                if (u.id === data.userId) {
                    return {
                        ...
                        active24h: data.isOnline ? true : u.active24h
                    };
                }
                return u;
            }));
    ...
    // Modal Action Handlers to update list instantly without refresh
                ...
                >
                    <div className="flex justify-between items-start">
                        <div>
                            ...
                            <h3 className={`text-3xl font-black mt-1 ${filter === 'all' ? 'text-white' : 'text-foreground'}`}>
                ...
                >
                    <div className="flex justify-between items-start">
                        <div>
                            ...
                            <h3 className={`text-3xl font-black mt-1 ${filter === '24h' ? 'text-white' : 'text-foreground'}`}>
                                {isLoading ? "-" : active24hCount}
                            </h3>
                        </div>
            ...
            <div className="bg-white border border-border/40 rounded-2xl md:rounded-3xl shadow-sm overflow-hidden flex flex-col">
                ...
                {isLoading ? (
                    <div className="flex items-center justify-center p-12 text-muted-foreground">
                        <Loader2 className="h-8 w-8 animate-spin text-[#2A5244]" />
                    </div>
                ) : (
                    <>
                        {/* Desktop Table View */}
                                ...
                                <tbody className="divide-y divide-border/20">
                                    {filteredUsers.length > 0 ? filteredUsers.map((user) => (
                                        <tr key={user.id} onClick={() => setSelectedUser(user)} className="hover:bg-[#FDFBF7] transition-colors cursor-pointer group">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="relative">
                                                        {user.profilePic ? (
                                                            <img src={user.profilePic} alt={user.name} className={`h-8 w-8 shrink-0 rounded-full object-cover border ${user.status === 'banned' ? 'border-red-500' : 'border-transparent'}`} />
                                                        ) : (
                                            ...
                                            <td className={`px-6 py-4 font-medium ${user.isOnline ? 'text-emerald-600 font-bold' : 'text-muted-foreground'}`}>
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            <div className="relative">
                                                {user.profilePic ? (
                                                    <img src={user.profilePic} alt={user.name} className={`h-10 w-10 shrink-0 rounded-full object-cover border ${user.status === 'banned' ? 'border-red-500' : 'border-transparent'}`} />
                                                ) : (
                                                ...
                                                {/* ✨ LIVE GREEN DOT MOBILE */}
                                                {user.isOnline && (
                                                    <span className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full bg-emerald-500 border-[2.5px] border-white shadow-sm" />
                                                )}
                                            </div>
                                            <div className="flex flex-col">
                                                <span className="font-bold text-foreground text-sm leading-tight truncate max-w-35">{user.name}</span>
                                                <span className={`text-[10px] font-medium mt-0.5 ${user.isOnline ? 'text-emerald-600' : 'text-muted-foreground'}`}>
                                                    {formatLastSeen(user.lastSeenAt, user.isOnline)}
                                                </span>
                                            </div>
                                        </div>
